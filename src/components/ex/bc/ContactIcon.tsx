import { Mail, MapPin, Smartphone, Globe } from "lucide-react";
import type { ContactKind } from "@/lib/ex/bc/types";

/**
 * Brand glyphs are hand-drawn (filled, like the original FontAwesome set);
 * the generic ones come from lucide so they stay consistent app-wide.
 */
...
function WhatsappGlyph({ size }: { size: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M17.47 14.38c-.3-.15-1.75-.86-2.02-.96-.27-.1-.47-.15-.67.15-.2.3-.77.96-.94 1.16-.17.2-.35.22-.64.07-.3-.15-1.25-.46-2.38-1.47-.88-.78-1.47-1.75-1.65-2.05-.17-.3-.02-.46.13-.6.14-.14.3-.35.45-.53.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.08-.15-.67-1.6-.92-2.2-.24-.58-.49-.5-.67-.5l-.57-.01c-.2 0-.52.07-.79.37-.27.3-1.04 1.01-1.04 2.47s1.06 2.86 1.21 3.06c.15.2 2.1 3.2 5.08 4.49.71.3 1.26.49 1.69.63.71.22 1.36.19 1.87.12.57-.09 1.75-.72 2-1.41.25-.7.25-1.29.17-1.41-.07-.13-.27-.2-.57-.35ZM12.05 21.7h-.01a9.6 9.6 0 0 1-4.9-1.34l-.35-.21-3.64.96.97-3.55-.23-.36a9.6 9.6 0 0 1-1.47-5.13c0-5.3 4.32-9.61 9.63-9.61a9.55 9.55 0 0 1 6.8 2.82 9.53 9.53 0 0 1 2.82 6.8c0 5.3-4.32 9.62-9.62 9.62ZM20.5 3.49A11.66 11.66 0 0 0 12.05 0C5.6 0 .36 5.24.36 11.68c0 2.06.54 4.06 1.56 5.83L.26 24l6.63-1.74a11.66 11.66 0 0 0 5.16 1.24h.01c6.44 0 11.68-5.24 11.68-11.68 0-3.12-1.22-6.05-3.42-8.26Z" />
    </svg>
  );
}

export function ContactIcon({ kind, size }: { kind: ContactKind; size: number }) {
  switch (kind) {
    case "facebook":
    ...
    case "location":
      return <MapPin size={size} strokeWidth={1.6} aria-hidden="true" />;
    case "website":
      return <Globe size={size} strokeWidth={1.6} aria-hidden="true" />;
    case "phone":
    default:
      return <Smartphone size={size} strokeWidth={1.6} aria-hidden="true" />;
  }
}
